...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }) {
  ...
  return {
    title: `${t('editCompanyType')} - HRM Admin`,
  };
}

export default async function EditCompanyTypePage({
  params: { locale, id },
}: {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6">
          <CompanyTypeFormContainer companyTypeId={Number(id)} locale={locale} />
        </CardContent>
      </Card>
    </div>
  );
